...
2. `Orchestrator` -> Receives Input -> Evaluates Intent -> Emits `PROCESS.AGENT_TRIGGERED`.
3. `Worker` -> Enqueues Job -> Executes `Agent Engine`.
4. `Agent Engine` -> Calls `Context Bus` -> Reasoning -> Decision -> `Tool Executor`.
5. `Tool Executor` -> Runs Tool (e.g., `check_availability`, `create_booking`) -> Emits `OUTPUT` event.
...
- **Bookings**: Scheduled appointments and status.
- **Agent Logs**: Traceable event chain history.
- **Agent Memories**: Historical execution patterns.
