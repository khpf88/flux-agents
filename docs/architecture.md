...
## 2. Intent Classification Agent (The Router)
- All new leads are first processed by the **Intent Classifier Agent**.
- **Pure Event Routing**: The Orchestrator does not call services directly; it routes via the `PROCESS.AGENT_TRIGGERED` event.
- **Agent ID Normalization**: Automatically converts human-friendly agent names into normalized file IDs (e.g., "Scheduling Agent" -> `scheduler_agent`).
- **Confidence Guardrails**: Routing falls back to general follow-up if confidence < 60%.
...
4. `Orchestrator` -> Receives Intent -> Emits `AGENT_TRIGGERED` for target agents.
5. `Worker` -> Enqueues Job -> Executes Agent Engine.
